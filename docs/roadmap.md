...
### B-02. 마일리지 → **v1.1로 이관 (V1.1-9)**

> 커머스 고객 대면 기능이라 Roadmap v1.1로 이관됨. 상세는 [V1.1-9](#v11-9-마일리지-b-02에서-이관) 참고.
...
### B-03. 쿠폰 → **v1.1로 이관 (V1.1-10)**

> 커머스 고객 대면 기능이라 Roadmap v1.1로 이관됨. 상세는 [V1.1-10](#v11-10-쿠폰-b-03에서-이관) 참고.
...
### B-04. 프론트엔드 TypeScript 마이그레이션 ✅ **완료** (PR #68/#69)

> 완료: 전 소스 39개(.tsx 24 + .ts 17) 전환, 공용 타입 `src/types/index.ts` 신설, tsc 0에러·빌드 성공, 브라우저 런타임 전 화면 검증(깨짐 0). 동작 불변.
...
> 마일리지(B-02)·쿠폰(B-03)은 v1.1로 이관(V1.1-9/10). 카테고리(B-05)·판매자(B-01)·TS 전환(B-04)은 완료.
...
| V1.1-5 | 반품·환불 | 중~대 | ✅ 완료 | 주문 라이프사이클 완성 |
| V1.1-6 | 결제(PG) 연동 | 대 | ⬜ 예정 | mock 결제 → 실제 결제 흐름 |
| V1.1-7 | 관리자 매출 통계 대시보드 | 중 | ✅ 완료 | 운영 가시성 |
| V1.1-8 | 상품 검색 고도화(자동완성·정렬) | 소~중 | ✅ 완료 | 탐색성 |
| V1.1-9 | 마일리지 | 중 | ⬜ 예정 | 리텐션·재구매 (B-02에서 이관) |
| V1.1-10 | 쿠폰 | 중~대 | ⬜ 예정 | 프로모션·전환 (B-03에서 이관) |

> B-05 카테고리 관리(ADMIN)·B-01 판매자·B-04 프론트 TS 마이그레이션도 완료됨(백로그 항목). 각 완료 기능의 상세 설계는 [docs/design/](design/) 참고.
> 마일리지(V1.1-9)·쿠폰(V1.1-10)은 커머스 고객 대면 기능이라 백로그(B-02/B-03)에서 v1.1로 이관. 둘 다 결제(V1.1-6)와 연계(할인·포인트가 결제금액에 적용).
...
### V1.1-9. 마일리지 (B-02에서 이관)
**개요**: 주문 완료 시 결제금액의 일정 비율 마일리지 적립, 다음 주문 시 사용.

**필요 작업**
- [ ] `mileage-service` 신규 생성 (또는 auth/order 확장)
- [ ] `Mileage` 엔티티 — userId, amount, type(EARN/USE), createdAt
- [ ] Kafka 이벤트 연동 — 주문 확정(OrderConfirmed) 수신 후 1% 적립
- [ ] `GET /api/v1/mileage/me` 잔액·내역, `POST /api/v1/mileage/use` 사용(주문서 호출)
- [ ] 프론트: 주문 화면 마일리지 사용 UI, 내 정보 잔액 표시
- **연계**: 결제(V1.1-6) 금액에 차감 적용.

### V1.1-10. 쿠폰 (B-03에서 이관)
**개요**: ADMIN이 쿠폰 발행, USER가 다운로드 후 주문 시 적용(1회용).

**필요 작업**
- [ ] `coupon-service` 신규 생성
- [ ] `Coupon`(코드·타입·금액·유효기간·수량) / `UserCoupon`(보유·사용) 엔티티
- [ ] `POST /api/v1/coupons`(ADMIN), `POST /api/v1/coupons/{id}/download`, `GET /api/v1/coupons/me`
- [ ] 주문 생성 시 쿠폰 코드 → order가 coupon-service 검증 호출 → 할인 적용
- [ ] 프론트: 쿠폰함, 주문 시 쿠폰 선택 UI
- **연계**: 결제(V1.1-6) 금액에 할인 적용.
...
> v1.1 잔여(반품·환불 V1.1-5 / 결제 PG V1.1-6 / 매출 통계 V1.1-7 / 마일리지 V1.1-9 / 쿠폰 V1.1-10)도 함께 우선순위 조정 대상. (B-02/B-03은 v1.1로 이관 완료, B-04 TS는 완료)
...
## Roadmap v2.0 — 글로벌화 트랙

> **성격이 다른 트랙**: v1.x가 "기능 추가"라면 v2.0은 **아키텍처 변경**이다. 규모는 v1.1 잔여 기능(PG결제·마일리지·쿠폰)을 합친 것보다 크며, 기존 기능 전반의 회귀 검증을 수반한다.
> 현재는 **한국 단일 시장 전제**로 의도적 미적용 — 배경·영향도·확장 경로는 [MD-06 트레이드오프 문서](tradeoffs/MD-06-globalization-scope.md) 참고.

### 왜 별도 트랙인가
- 기능 목록(v1.1/v1.2)에 끼우면 성격이 섞인다. TS 마이그레이션을 v1.1에 넣지 않은 것과 같은 이유.
- **다국어는 늦게 할수록 비용이 늘지 않는다**(작업량 = 문자열 × 화면). 화면 확정 후 일괄 처리가 효율적이며, 먼저 도입하면 이후 모든 신규 기능에 번역 키 관리 부담이 붙는다.
- **타임존은 반대로 늦을수록 비싸진다**(시간 필드를 쓰는 기능이 늘수록 수정 범위 확대). → **v2.0 내에서도 타임존을 먼저** 처리한다.

### 진행 순서 (의존성 기준)

| # | 항목 | 규모 | 선행 | 핵심 변경 |
|---|------|------|------|----------|
| V2.0-1 | **타임존 정규화** | 중 | — | DB `TIMESTAMP`→`TIMESTAMPTZ`, `LocalDateTime`→`Instant`(UTC 저장), 기존 데이터 마이그레이션, 집계 기준 타임존 정책 |
| V2.0-2 | **통화 다중화** | 중~대 | V2.0-1 | `long`→`BigDecimal`+통화코드(ISO 4217), 결제 시점 통화·환율 스냅샷, PG 통화별 지원 |
| V2.0-3 | **백엔드 메시지 i18n** | 중 | — | 예외 메시지 `messages_{locale}.properties` 분리, `Accept-Language` 기반 `MessageSource` |
| V2.0-4 | **UI 다국어** | 대 | V2.0-3 | `react-i18next` 도입, 전 화면 문자열 추출(화면 20+·수백 개) |
| V2.0-5 | **콘텐츠 다국어** | 대 | V2.0-4 | `product_translation(product_id, locale, name, description)` 등 DB 다국어 테이블, 판매자 입력 UI, 미번역 폴백, 로캘별 검색·자동완성 |
| V2.0-6 | **지역화 부수 항목** | 중 | V2.0-2 | 국가별 주소 체계·전화번호(현재 한국 전용 정규식), 배송비·세금(VAT), 국가별 법적 고지 |

### V2.0-1 타임존 정규화 — 상세
- [ ] DB 마이그레이션: `TIMESTAMP` → `TIMESTAMPTZ` (기존 값을 KST로 해석해 UTC 변환)
- [ ] 엔티티 `LocalDateTime` → `Instant` 전환 — 주문·배송상태·반품(requested/processed/refunded)·알림·리뷰·장바구니·실패주문로그
- [ ] **집계 정책 재정의**: "7월 매출"의 기준 타임존 확정(일반적으로 상점 기준 고정), 통계 API에 타임존 파라미터 또는 상점 설정
- [ ] 프론트: 사용자 로캘·타임존 기준 날짜 표시
- [ ] 회귀 검증: 주문 Saga·통계·반품·알림 전 영역
- **확장 지점**: 현재 `ServiceTimeZone` 상수 1곳에서 타임존을 관리 → 전환 시작점이 명확

### V2.0-2 통화 다중화 — 상세
- [ ] 금액 타입 `long`(원) → `BigDecimal` + 통화코드 + 최소단위(minor unit)
- [ ] 상품 가격의 통화별 저장 vs 기준통화+환율 변환 결정(환율 스냅샷 보관 포함)
- [ ] 주문·결제 금액은 **결제 시점 통화·환율 스냅샷**으로 이력 보존
- [ ] PG 연동(V1.1-6)과 함께 결정하는 것이 유리 — 통화별 지원 범위가 다름

### V2.0-3~5 다국어 — 상세
- [ ] (V2.0-3) 백엔드: ProblemDetail `detail` 등 한국어 하드코딩 메시지를 로캘 리소스로 분리
- [ ] (V2.0-4) 프론트: i18n 프레임워크 도입 + 전 화면 문자열 키 추출, 언어 전환 UI
- [ ] (V2.0-5) 콘텐츠: 상품·카테고리 다국어 테이블, 판매자 언어별 입력, 미번역 폴백 정책, 검색·자동완성 로캘 대응
- [ ] 이메일 템플릿(임시 비밀번호 등) 로캘별

### 결정 기록
- **2026-08-10**: 글로벌화를 v1.1/v1.2가 아닌 **v2.0 별도 트랙**으로 확정. 근거는 위 "왜 별도 트랙인가" 및 [MD-06](tradeoffs/MD-06-globalization-scope.md). 포트폴리오 범위에서는 **한국 단일 시장 전제 유지**, 확장 경로만 설계로 남긴다.
