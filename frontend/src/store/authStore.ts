...
import type { AuthTokenResponse, JwtClaims, Role } from '../types';
...
interface AuthState {
  ...
  logout: () => void;
  tryRestoreAuth: () => Promise<void>;
}

const UNAUTHENTICATED = { isAuthenticated: false, userId: null, role: null, pwdChangeRequired: false } as const;
...
const useAuthStore = create<AuthState>()((set) => ({
  ...
  tryRestoreAuth: () => {
    ...
    restoreInFlight = (async () => {
      try {
        const res = await api.post<AuthTokenResponse>('/api/v1/auth/refresh');
