...
- 형식: `<type>(<scope>): <subject>` ([git-convention.md](git-convention.md) 참조)
- 예: `feat(auth): 로그인 JWT 발급 구현`
- Squash 머지 시 PR 제목이 그대로 커밋 메시지
...
## 영향 서비스
- `auth` / `product` / `order` / `gateway` / `frontend` / `infra` 중 변경된 곳 명시
...
## 라벨 정책
- `type:feat` / `type:fix` / `type:chore` / `type:docs` 등 타입 라벨
- `service:auth` / `service:product` / `service:order` 등 영향 서비스 라벨
- `breaking-change` — 호환성 깨지는 변경
- `needs-review` / `wip` 상태 라벨
