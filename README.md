...
| Frontend | React 19 + TypeScript 5, Vite 8, Axios, Zustand, React Router v7, Tailwind CSS |
...
| Order Service | 8083 | 주문 처리(Saga) + 항목/주문 취소 + 배송지·배송상태 + 알림 + 반품·환불 + 관리자 매출 통계 + Kafka 이벤트 |
| Monitoring | 8084 | 서비스 헬스 대시보드 API (관리자 전용) |
| Frontend | 5173 | React + TypeScript SPA (Vite dev 서버) |
...
- **알림** — 주문 확정/취소/배송상태 변경, 반품 승인/거부/환불 시 인앱 알림, 미읽음 뱃지
- **반품·환불** — 배송완료 항목 반품 신청, 관리자/판매자 승인(재고 복구 + 환불)·거부(사유 필수), 중복 신청·동시 처리 방어(409)
- **관리자 매출 통계** — 기간 요약(매출·주문수·AOV·취소율), 일별 추이, 상품/판매자 Top N, 실패주문 추이 (ADMIN 전용, KST 기준 집계)
...
- [로드맵](docs/roadmap.md)
- [기능 설계 문서](docs/design/) — 위시리스트·검색·리뷰별점·배송지/배송상태·알림·반품환불·매출통계
- [배포 체크리스트](docs/deploy-checklist.md)
- [기술 선택 근거](docs/decisions/)
- [트레이드오프](docs/tradeoffs/OVERVIEW.md) — 의도적으로 범위 밖에 둔 선택과 근거
